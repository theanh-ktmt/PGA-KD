...
from src.model.processor import (LLAVA_NEXT, QWEN2_VL, PHI3V, get_backbone_name, print_master, QWEN2_5_VL, 
    ...
    GME, VLM_IMAGE_TOKENS, LamRA, COLPALI, INTERN_VL3)
...
class MMEBModel(nn.Module):
    """
    Multimodal Embedding Benchmark (MMEB) Model class.
    
    This class wraps various vision-language backbones and provides a unified interface for 
    encoding multimodal inputs and computing similarity scores.
    """
    ...
    def __init__(self,
        ...
        """
        Initialize the MMEBModel.

        Args:
            encoder (PreTrainedModel): The backbone encoder model.
            pooling (str): Pooling strategy to use ('last', 'mean', etc.). Defaults to 'last'.
            normalize (bool): Whether to normalize the embeddings. Defaults to False.
            temperature (float): Temperature parameter for contrastive loss. Defaults to 0.02.
        """
        super().__init__()
    ...
    def encode_input(self, input):
        """
        Encode the input dictionary into embeddings.

        Args:
            input (Dict): Dictionary containing input tensors (input_ids, pixel_values, etc.).

        Returns:
            Tensor or Tuple: The encoded embeddings or a tuple containing (pooled_output, image_features, attention_matrix, output_hidden_states).
        """
    ...
    def _pooling(self, last_hidden_state, attention_mask):
        """
        Pool the hidden states to get a single vector representation.

        Args:
            last_hidden_state (Tensor): The sequence of hidden-states at the output of the last layer of the model.
            attention_mask (Tensor): Mask to avoid performing attention on padding token indices.

        Returns:
            Tensor: Pooled representation.
        """
    ...
    def build(cls, model_args: ModelArguments, **kwargs):
        """
        Build a new MMEBModel instance based on model arguments.

        Args:
            model_args (ModelArguments): Arguments for model configuration.
            **kwargs: Additional keyword arguments passed to the model initialization.

        Returns:
            MMEBModel: The built model instance.
        """
        INTERNVIDEO2 = "internvideo2"
        ...
        if model_args.load_pretrained_lora:
            ...
            if model_args.model_backbone in ["llava_onevision", "llava_next"]:
                ...
                # Explicitly list all modules inside language_model
                explicit_target_modules = []
                
                for name, module in base_model.named_modules():
                    # Only get modules with "language_model" in the name
                    if 'language_model' in name:
                        module_name = name.split('.')[-1]
                        if module_name in base_targets:
                            # Add full path of this module
                ...
                lora_config = LoraConfig(
                    ...
                    target_modules=explicit_target_modules,  # Use full paths
    ...
    def load(cls, model_args: ModelArguments, is_trainable=True, **kwargs):
        """
        Load a pretrained MMEBModel.

        Args:
            model_args (ModelArguments): Arguments for model configuration.
            is_trainable (bool): Whether the LoRA adapter should be trainable. Defaults to True.
            **kwargs: Additional keyword arguments.

        Returns:
            MMEBModel: The loaded model instance.
        """
        # Loading the base model
        INTERNVIDEO2 = "internvideo2"
        model_name_or_path = model_args.checkpoint_path if model_args.checkpoint_path else model_args.model_name
    ...
    def forward(self, qry: Dict[str, Tensor] = None, tgt: Dict[str, Tensor] = None, *args, **kwargs):
        """
        Forward pass for computing contrastive loss between queries and targets.

        Args:
            qry (Dict[str, Tensor], optional): Dictionary containing query inputs.
            tgt (Dict[str, Tensor], optional): Dictionary containing target inputs.

        Returns:
            Tensor: The computed loss or a dictionary of representations if one of the inputs is None.
        """
    ...
    def _dist_gather_tensor(self, t: Tensor):
        """
        Gather tensors from all processes in a distributed setting.

        Args:
            t (Tensor): The tensor to gather.
    ...
    def compute_similarity(self, q_reps, p_reps):
        """
        Compute similarity scores between query and passage representations.

        Args:
            q_reps (Tensor): Query representations.
            p_reps (Tensor): Passage/Target representations.
